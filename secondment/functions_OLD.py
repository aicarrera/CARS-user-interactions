...
'''
'''
def table(filenames,filenames2, metric, folder="results/"):
    all_results = pd.DataFrame()
    for f in filenames:
        # Load the results into a DataFrame
        results_df = pd.read_csv(folder + f)
    ...
    for f in filenames2:
        # Load the results into a DataFrame
        results_df = pd.read_csv("results/withoutContext/"+f)
    ...
    grouped = all_results.groupby(['context', 'distance', 'k']).mean()[['mae']]
    grouped = grouped.reset_index()
    df_pivot = grouped.pivot(index='k', columns=["context",'distance'], values='mae')
    print(df_pivot)
...
if len(filenames)>0:
    table(filenames,filenames2,"precision","results/withContextFinal/")
'''